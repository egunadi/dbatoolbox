...
INSERT INTO MCdbWAITS 
  SELECT '<a href="https://app.spotlightcloud.io/waitopedia/waits/'+LOWER(MAX ([W1].[wait_type]))+'">
...
INSERT INTO MCdbMEMCLERK 
  SELECT TOP(10) '<br>'+LOWER(mc.[type])+' -- '+ 
...
SELECT total=COUNT(sent_status),sent_status 
INTO #MAILCOUNT 
FROM msdb.dbo.sysmail_allitems 
WHERE mailitem_id IN 
  (SELECT TOP 50 mailitem_id FROM msdb.dbo.sysmail_allitems ORDER BY mailitem_id DESC) 
...
GO 
INSERT INTO MCdbMAILSTATUS 
  SELECT 'db-Mail Status'=CASE WHEN (
...
INSERT INTO MCdbAVGTEMPSTALL 
  SELECT 'Average Temb db Stall'=rtrim(convert(char,cast(avg(1.0 * stats.io_stall_read_ms / stats.num_of_reads) 
    AS DECIMAL (5,2))))+' (ms)' 
  FROM master.sys.dm_io_virtual_file_stats(2, NULL) as stats
...
WITH [Blocking] AS 
(
  SELECT w.[session_id],s.[original_login_name],s.[login_name],w.[wait_duration_ms],w.[wait_type]
...
INSERT INTO MCdbBLOCKEDSESSIONS 
  SELECT [WaitingSessionID]='<br>'+CONVERT(CHAR,b.[session_id])+''+b.[blocking_session_id]+''
